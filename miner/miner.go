...
func New(taichain Backend, config *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine,
	...
	miner := &Miner{
		...
		worker:            newWorker(config, engine, common.Address{}, taichain, mux),
	...
	if !remoteMining {
		miner.Register(NewCPUAgent(taichain.SnailBlockChain(), engine))
		if !miner.singleNode {
