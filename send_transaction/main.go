...
func sendBalanceNewAccount(client *rpc.Client, count int, main *big.Int) bool {
	...
	averataipublic := new(big.Int).Set(average)
	fmt.Println("sendBalanceNewAccount ", " true ", averataipublic.Div(averataipublic, big.NewInt(1000000000000000000)), " average ", average, " hex ", value)
...
func checkSonAccountBalance(client *rpc.Client, count int, main *big.Int) bool {
	...
	value := "0x" + fmt.Sprintf("%x", average)
	averataipublic := new(big.Int).Set(average)
	fmt.Println("checkSonAccountBalance ", " true ", averataipublic.Div(averataipublic, big.NewInt(1000000000000000000)), " average ", average, " hex ", value)
