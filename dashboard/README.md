...
The dashboard is a data visualizer integrated into taipublic, intended to collect and visualize useful information of an Ethereum node. It consists of two parts:
...
Normally the dashboard assets are bundled into Taipublic via `go-bindata` to avoid external dependencies. Rebuilding Taipublic after each UI modification however is not feasible from a developer perspective. Instead, we can run `yarn dev` to watch for file system changes and refresh the browser automatically.

```
$ taipublic --dashboard --vmodule=dashboard=5
$ (cd dashboard/assets && yarn dev)
```

To bundle up the final UI into Taipublic, run `go generate`:
