...
import (
	...
	"github.com/taiyuechain/taipublicchain/tai/fetcher"
	"github.com/taiyuechain/taipublicchain/tai/fetcher/snail"
...
// NewProtocolManager returns a new Taichain sub protocol manager. The Taichain sub protocol manages peers capable
// with the Taichain network.
func NewProtocolManager(config *params.ChainConfig, mode downloader.SyncMode, networkID uint64, mux *event.TypeMux, txpool txPool, SnailPool SnailPool, engine consensus.Engine, blockchain *core.BlockChain, snailchain *snailchain.SnailBlockChain, chaindb taidb.Database, agent *PbftAgent) (*ProtocolManager, error) {
...
func (pm *ProtocolManager) removePeer(id string, call uint32) {
	...
	log.Debug("Removing Taichain peer", "peer", id, "recipients", len(pm.peers.peers))

	// Unregister the peer from the downloader and Taichain peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping Taichain protocol")
	...
	log.Info("Taichain protocol stopped")
}
...
func resolveVersionFromName(name string) bool {
	str := name
	flag := "Taipublic/v0.8.2"
	if !strings.Contains(str, "Taipublic/v0.8") {
		return true
	}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Taichain peer connected", "name", p.Name(), "remoteAddr", p.RemoteAddr())

	// Execute the Taichain handshake
	var (
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Taichain peer registration failed", "err", err)
		return err
	}
	...
	if err := pm.downloader.RegisterPeer(p.id, p.version, p.RemoteAddr().String(), p); err != nil {
		p.Log().Error("Taichain downloader registerPeer registration failed", "err", err)
		return err
	}

	if err := pm.fdownloader.RegisterPeer(p.id, p.version, p); err != nil {
		p.Log().Error("Taichain fdownloader registerPeer registration failed", "err", err)
		return err
	}
	...
	for {
		err := pm.handleMsg(p)
		if err != nil {
			p.Log().Info("Taichain message handling failed", "remoteAddr", p.RemoteAddr(), "err", err)
			return err
		}
	}
}
...
// NodeInfo represents a short summary of the Taichain sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network      uint64              `json:"network"`      // Taichain network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
