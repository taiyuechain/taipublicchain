...
// Package utils contains internal helper functions for taipublicchain commands.
package utils
...
import (
	...
	"github.com/taiyuechain/taipublicchain/accounts/keystore"
	"github.com/taiyuechain/taipublicchain/common"
	"github.com/taiyuechain/taipublicchain/common/fdlimit"
	"github.com/taiyuechain/taipublicchain/consensus"

	//"github.com/taiyuechain/taipublicchain/consensus/clique"
	"bytes"
	...
	"github.com/taiyuechain/taipublicchain/les"
	"github.com/taiyuechain/taipublicchain/log"
	"github.com/taiyuechain/taipublicchain/metrics"
	"github.com/taiyuechain/taipublicchain/metrics/influxdb"
	...
	"github.com/taiyuechain/taipublicchain/p2p/nat"
	"github.com/taiyuechain/taipublicchain/p2p/netutil"
	...
	"github.com/taiyuechain/taipublicchain/taistats"
...
// for taipublic and returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses!(can search via `taipublic account list`)")
	log.Warn("-------------------------------------------------------------------")
...
// RegisterEtrueService adds an Taichain client to the stack.
func RegisterEtrueService(stack *node.Node, cfg *tai.Config) {
	...
	if err != nil {
		Fatalf("Failed to register the Taichain service: %v", err)
	}
}
...
// RegisterEtrueStatsService configures the Taichain Stats daemon and adds it to
// th egiven node.
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "taipublic.", map[string]string{
				"host": hosttag,
			})
		}
	}
...
func MakeChain(ctx *cli.Context, stack *node.Node) (fchain *core.BlockChain, schain *snailchain.SnailBlockChain, chainDb taidb.Database) {
	var err error
...
// new format.
//
// e.g. taipublic account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// taipublic --keystore /tmp/mykeystore --lightkdf account new
//
