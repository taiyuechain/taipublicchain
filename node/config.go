...
import (
	...
	"github.com/taiyuechain/taipublicchain/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of taipublic is "taipublic". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "taipublic", keep that.
	if name == "taipublic" || name == "taipublic-testnet" {
		name = "Taipublic"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "taipublic" instances.
...
func (c *Config) ResolvePath(path string) string {
	...
	// by taipublic 1.4 are used if they exist.
	if c.name() == "taipublic" && isOldGethResource[path] {
		oldpath := ""
		if c.Name == "taipublic" {
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "taipublic-keystore")
		ephemeral = keydir
	}
