...
// PeerInfo represents a short summary of the Taichain sub-protocol metadata known
// about a connected peer.
type PeerInfo struct {
	Version    int      `json:"version"`    // Taichain protocol version negotiated
...
// Handshake executes the tai protocol handshake, negotiating version number,
...
func (p *peer) String() string {
	return fmt.Sprintf("Peer %s [%s]", p.id,
		fmt.Sprintf("tai/%2d", p.version),
	)
}

// peerSet represents the collection of active peers currently participating in
// the Taichain sub-protocol.
type peerSet struct {
