...
import (
	...
	"github.com/taiyuechain/taipublicchain/internal/jsre"
	"github.com/taiyuechain/taipublicchain/internal/web3ext"
...
func (c *Console) init(preload []string) error {
	...
	flatten := "var etai = web3.etai; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = etai.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines etai.getBalance(etai.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current taipublic instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	// Print some generic taipublic metadata
	fmt.Fprintf(c.printer, "Welcome to the Taipublic JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + etai.coinbase);
		console.log("at block: " + etai.blockNumber + " (" + new Date(1000 * etai.getBlock(etai.blockNumber).timestamp) + ")");
