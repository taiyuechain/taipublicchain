...
import (
	"errors"
	"fmt"
	"github.com/taiyuechain/taipublicchain/consensus/tbft/help"
	"sync"
	"sync/atomic"
	"time"
...
type SDownloader interface {
	SyncStateFd(root common.Hash) tai.StateSyncInter
}
...
type Downloader struct {
	...
	receiptCh     chan tai.DataPack    // [eth/63] Channel receiving inbound receipts
...
func New(mode SyncMode, stateDb taidb.Database, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer tai.PeerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	...
	dl := &Downloader{
		...
		receiptCh:     make(chan tai.DataPack, 1),
...
func (d *Downloader) SetSync(StateSync tai.StateSyncInter) {
...
func (d *Downloader) RegisterPeer(id string, version int, peer tai.Peer) error {
...
func (d *Downloader) RegisterLightPeer(id string, version int, peer tai.LightPeer) error {
...
func (d *Downloader) synchronise(id string, hash common.Hash, mode SyncMode, origin uint64, height uint64) error {
	...
	for _, ch := range []chan tai.DataPack{d.headerCh, d.bodyCh, d.receiptCh} {
...
func (d *Downloader) syncWithPeer(p tai.PeerConnection, hash common.Hash, origin uint64, height uint64) (err error) {
...
func (d *Downloader) fetchHeaders(p tai.PeerConnection, from uint64, height int, pivot uint64) error {
...
func (d *Downloader) fetchReceipts(from uint64) error {
	...
	var (
		...
		fetch    = func(p tai.PeerConnection, req *tai.FetchRequest) error { return p.FetchReceipts(req) }
		capacity = func(p tai.PeerConnection) int { return p.ReceiptCapacity(d.requestRTT()) }
		setIdle  = func(p tai.PeerConnection, accepted int) { p.SetReceiptsIdle(accepted) }
	)
...
func (d *Downloader) fetchParts(errCancel error, deliveryCh chan tai.DataPack, deliver func(tai.DataPack) (int, error), wakeCh chan bool,
	...
	fetchHook func([]*types.Header), fetch func(tai.PeerConnection, *tai.FetchRequest) error, cancel func(*tai.FetchRequest), capacity func(tai.PeerConnection) int,
...
func (d *Downloader) importBlockResults(results []*tai.FetchResult) error {
...
func splitAroundPivot(pivot uint64, results []*tai.FetchResult) (p *tai.FetchResult, before, after []*tai.FetchResult) {
...
func (d *Downloader) commitPivotBlock(result *tai.FetchResult) error {
...
func (d *Downloader) commitFastSyncData(results []*tai.FetchResult) error {
	// Check for any early termination requests
	if len(results) == 0 {
		return nil
	}
...
func (d *Downloader) deliverOne(id string, destCh chan tai.DataPack, packet tai.DataPack, inMeter, dropMeter metrics.Meter) (err error) {
	// Update the delivery metrics for both good and failed deliveries
