...
func (c *Console) init(preload []string) error {
	...
	flatten := "var tai = web3.tai; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = tai.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines tai.getBalance(tai.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
func (c *Console) Welcome() {
	...
	c.jsre.Run(`
		...
		console.log("coinbase: " + tai.coinbase);
		console.log("at block: " + tai.blockNumber + " (" + new Date(1000 * tai.getBlock(tai.blockNumber).timestamp) + ")");
